(function () {
  ...
  // collides with each host page's own CSS. Posts same-origin to /api/rating -
  // one click, no message required, so it stays inviting rather than naggy.

  var RATED_KEY = 'dt_rating_done_at';
  var DISMISSED_KEY = 'dt_rating_dismissed_at';
  ...
  // Best-effort: several tools on this site stash the connected org's
  // instanceUrl under sessionStorage keys named oauth_<something>. Not every
  // page has one (a visitor may not be signed into any org), so this is
  // optional context for the email, not a requirement to send a rating.
  function detectOrg() {
    try {
      for (var i = 0; i < sessionStorage.length; i++) {
        ...
        if (parsed && parsed.instanceUrl) {
          return new URL(parsed.instanceUrl).hostname;
        }
      }
    } catch (e) { /* ignore, org is optional */ }
    return '';
  }

  var host = document.createElement('div');
  host.id = 'dt-rating-widget-host';
  ...
  function mount() {
    ...
    if (document.getElementById('dt-rating-widget-host')) return;
